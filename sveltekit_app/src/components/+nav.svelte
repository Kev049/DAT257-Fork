...
<nav class="w-full h-20 lg:h-24 grid grid-cols-3 bg-white border-4 border-[#333333]">
    ...
    <div class="flex justify-center items-center m-0 text-white text-4xl font-dosis">
        <form class="relative w-full flex max-h-full" on:submit="{handleFormSubmit}">
            <input type="search" bind:value={selectedCountry} on:input="{updateCountry}" class="block w-full p-4 pl-10 text-sm text-gray-900 border border-[#333333] rounded-lg bg-gray-200 outline-none font-dosis" placeholder="Search countries, continents..." required />
